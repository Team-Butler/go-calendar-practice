...
func (uq *UserQuery) QuerySchedules() *ScheduleQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, false, user.SchedulesTable, user.SchedulesPrimaryKey...),
		)
	...
	if query := uq.withSchedules; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(user.SchedulesPrimaryKey[0], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, uq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "schedules": %w`, err)
		}
		query.Where(schedule.IDIn(edgeids...))
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "schedules" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Schedules = append(nodes[i].Edges.Schedules, n)
			}
		}
	}
